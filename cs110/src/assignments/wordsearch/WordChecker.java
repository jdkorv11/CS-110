...
public class WordChecker {
	...
	public WordVector findWord(String[] horizontalTemplate,
		...
		if (wordStart[0] != -1) {
			wv = new WordVector(wordStart[0], wordStart[1], word.length(), Direction.EASTERLY);
			return wv;
		}
		wordStart = westerlyCheck(horizontalTemplate, word);
		if (wordStart[0] != -1) {
			wv = new WordVector(wordStart[0], wordStart[1], word.length(), Direction.WESTERLY);
			return wv;
		}
		wordStart = southerlyCheck(verticalTemplate, word);
		if (wordStart[0] != -1) {
			wv = new WordVector(wordStart[0], wordStart[1], word.length(), Direction.SOUTHERLY);
			return wv;
		}
		wordStart = northerlyCheck(verticalTemplate, word);
		if (wordStart[0] != -1) {
			wv = new WordVector(wordStart[0], wordStart[1], word.length(), Direction.NORTHERLY);
			return wv;
		}
		return null;
	}

	public int[] easterlyCheck(String[] template, String word) {
		int[] wordFoundAt = { -1, -1 };
	...
	public int[] westerlyCheck(String[] template, String word) {
		int[] wordFoundAt = { -1, -1 };
	...
	public int[] southerlyCheck(String[] template, String word) {
		int[] wordFoundAt = { -1, -1 };
		for (int i = 0; i < template.length; i++) {
			wordLocation = isWord(template[i], word);
			if (wordLocation != -1) {
	...
	public int[] northerlyCheck(String[] template, String word) {
		int[] wordFoundAt = { -1, -1 };
		word = new StringBuffer(word).reverse().toString();
		for (int i = 0; i < template.length; i++) {
			wordLocation = isWord(template[i], word);
	...
	public int isWord(String puzzleLine, String word) {
		int wordAt = -1;
